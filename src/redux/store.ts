...
// todo ///  useSelector - внутри этого хука вшит useContext, у него есть свой провайдер и контекст. Внутрь useSelector'a передаём функцию, которая принимает стейт и возращает то, что нам нужно, например (const categoryId = useSelector((state) => state.filter.categoryId))
