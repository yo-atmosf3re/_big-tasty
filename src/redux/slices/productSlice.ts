import { CartItem, ItemType } from './../../@types/types';
...
export const fetchProducts = createAsyncThunk<ItemType[], FetchProductsArgumentsType>('product/fetchProductsStatus', async (params) => {
   const { order, sortBy, categorySelection, search, pageCount } = params;
   const { data } = await axios.get<ItemType[]>(`https://633fd93ae44b83bc73c298e6.mockapi.io/items?page=${pageCount}&limit=4&${categorySelection}&sortBy=${sortBy}&order=${order}${search}`);
   return data;
})
